...
NRUNS = 5                             # Number of runs to run the model for
...
tsurf_init = 325.                       # initial surface temperature (K)
...
atm_bucket = 0.05                       # initial depth of water in atmosphere (m)
bucket_depth = 10.0 - atm_bucket        # Bucket depth, in meters
...
diag.add_field('atmosphere', 'sphum_dt1', time_avg=True)
diag.add_field('atmosphere', 'sphum_dt2', time_avg=True)
diag.add_field('atmosphere', 'sphum_dt3', time_avg=True)
diag.add_field('atmosphere', 'sphum_dt4', time_avg=True)
diag.add_field('atmosphere', 'sphum_dt5', time_avg=True)
diag.add_field('atmosphere', 'sphum_dt6', time_avg=True)
diag.add_field('atmosphere', 'sphum_dt7', time_avg=True)
diag.add_field('atmosphere', 'sphum_dt8', time_avg=True)
diag.add_field('atmosphere', 'sphum_dt9', time_avg=True)
diag.add_field('atmosphere', 'sphum_dt10', time_avg=True)

diag.add_field('tam_hydrology', 'totrain', time_avg=True)
diag.add_field('tam_hydrology', 'totevap', time_avg=True)
diag.add_field('tam_hydrology', 'qsfc1', time_avg=True)
diag.add_field('tam_hydrology', 'qsfc2', time_avg=True)
...
namelist = Namelist({
    ...
    'idealized_moist_phys_nml': {
        ...
        'couple_flux_q':False, #trying to couple flux_q to sphum
        'var_surf_prop':False, #whether to adjust albedo based on water in grid cell
        'vis_albedo':0.05 #albedo default, applies to all cells if var_surf_prop is False, to dry cells if True
    },
    ...
    'hydrology_nml':{
        ...
        'init_surf_height':1.0,
        'hyd_cond':0.0,
        'infil_rate':0.0 #setting the infil speeds to 0 to give effectively no infiltration
    },

    'tam_surface_nml':{
        'tsurf_init':tsurf_init,
        'cnfac':1.0               # default is 0.5
    },

    'tam_surface_flux_nml':{
        'sfc_heat_flx_amp':1.0    # want to see how this affects SH and LH
    },
