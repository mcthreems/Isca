...
water_lat = 25                          # latitude limit of dry equatorial band
topo_input = 'topo_high_eq_'+str(int(water_lat))+'.nc'
inputfiles = [os.path.join(GFDL_BASE,'exp/my_inputs/'+topo_input),os.path.join(GFDL_BASE,'exp/my_inputs/land_all_lowres.nc'),os.path.join(GFDL_BASE,'input/rrtm_input_files/ozone_1990.nc')]
...
do_v_temp = True                        # do virtual temp
do_simp = True                         # do_simple flag
atm_bucket = 0.0                        # initial depth of water in atmosphere (m)
bucket_depth = 30 - atm_bucket        # Bucket depth, in meters
isphum = atm_bucket*9.8/100.            # initial specific humidity (kg/kg)
beta = 1.0                              # beta parameter from Fan et al
es0 = 4                               # es0 as a factor of current Earth
omega_scale = 8                         # Scaling of rotation rate relative to Earth's (rotation in days)
...
namelist = Namelist({
    ...
    'vert_turb_driver_nml': {
        ...
        'do_simple': do_simp,             # default: False
    ...
    'hydrology_nml':{
        ...
        'init_surf_height':1.0, #height above impermeable basin, in meters (bucket_depth+atm_bucket)
        ...
        'water_lat_limit':water_lat      #making land between this latitude band dry to start
    },
